    ...
    const fetchUserDataAndRides = async () => {
      ...
      try {
        ...
        const { data: ridesData, error: ridesError } = await supabase
          .from("Vaam-Dashboard")
          .select("id, driver_id, pickup, destination, earning, rideTime, date, time")
    ...
    <main className="min-h-screen  p-6 md:p-8">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="grid gap-6 md:grid-cols-3">
          <Card className="bg-white dark:bg-black border border-gray-200 dark:border-[#ffd342]/50 shadow-lg hover:shadow-xl transition-shadow">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-gray-700 dark:text-gray-300">Today's Earnings</CardTitle>
              <Car className="h-5 w-5 text-[#ffd342]" />
            </CardHeader>
            <CardContent>
              <div className="text-3xl font-bold text-black dark:text-white">$245.00</div>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">+5% from yesterday</p>
            </CardContent>
          </Card>
          <Card className="bg-white dark:bg-black border border-gray-200 dark:border-[#ffd342]/50 shadow-lg hover:shadow-xl transition-shadow">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-gray-700 dark:text-gray-300">Total Rides</CardTitle>
              <Timer className="h-5 w-5 text-[#ffd342]" />
            </CardHeader>
            <CardContent>
              <div className="text-3xl font-bold text-black dark:text-white">12</div>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Completed today</p>
            </CardContent>
          </Card>
          <Card className="bg-white dark:bg-black border border-gray-200 dark:border-[#ffd342]/50 shadow-lg hover:shadow-xl transition-shadow">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-gray-700 dark:text-gray-300">Online Hours</CardTitle>
              <MapPin className="h-5 w-5 text-[#ffd342]" />
            </CardHeader>
            <CardContent>
              <div className="text-3xl font-bold text-black dark:text-white">8.5h</div>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Today's active time</p>
            </CardContent>
          </Card>
        </div>
        ...
        <Card className="bg-white dark:bg-black border  rounded-xl shadow-lg">
          <CardHeader>
            <CardTitle className=" flex justify-center text-2xl font-semibold text-black dark:text-white">Recent Rides</CardTitle>
          </CardHeader>
          <CardContent>
                ...
                rides.map((ride, index) => (
                  <div
                    ...
                    className={`p-5 bg-white dark:bg-black rounded-lg border   shadow-lg hover:shadow-lg transition-shadow ${
                      index !== rides.length - 1 ? "border-b-0 dark:border-b-0" : ""
                    }`}
                  >
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center text-sm">
                        ...
                        <span className="text-black dark:text-white">{ride.pickup}</span>
                      </div>
                      ...
                      <div className="flex items-center text-sm">
                        <span className="w-3 h-3 bg-[#ffd342] rounded-full mr-2"></span>
                        <span className="text-black dark:text-white">{ride.destination}</span>
                      </div>
                    </div>
